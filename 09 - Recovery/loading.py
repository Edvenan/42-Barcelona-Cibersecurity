...
def ft_progress(lst):
    ...
    for element in lst:
        ...
        if percentage < 100:
            print("Checking records...ETA: {:>8} [{:>3,.0f}%] [{:<20}] {}/{} | elapsed time {}".format(str(eta), percentage, progress, iterations, total_iterations, str(elapsed_time)), end="\r")
        else:
            # When percentage = 100%, remove leading space of percentage field and Print dotted line 
            print("Checking completed.ETA: {:>8} [{:>3,.0f}%] [{:<20}] {}/{} | elapsed time {}".format(str(eta), percentage, progress, iterations, total_iterations, str(elapsed_time)), end="\r")
            print("\n...", end='')
